...
def draw_grid(surface, grid): #Draws the grid lines on the surface
    sx = top_left_x
    sy = top_left_y
    for i in range(len(grid)):
...
def draw_next_shape(shape, surface):
    ...
    sx = top_left_x + play_width + 50 #TOOD: Change these constants as wanted
...
def main(win):
    ...
    fall_time = 0
    fall_speed = 0.27  # controls how fast the pieces fall
    level_time = 0
    score = 0
    ...
    while run:
        ...
        if level_time / 1000 >= 5:  # every 5 seconds increase the fall speed
            level_time = 0
            if fall_speed > 0.12:  # minimum fall speed
                fall_speed -= 0.005  # decrease the fall speed
